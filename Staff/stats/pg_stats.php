...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM infos_match WHERE id_equipe = :id");
$stmt->execute(['id' => $id_equipe]);
$stats['total_matchs'] = $stmt->fetchColumn();

// Victories by this team
$stmt = $pdo->prepare("SELECT COUNT(*) FROM infos_match WHERE id_equipe = :id AND match_gagnant = 'asbh'");
$stmt->execute(['id' => $id_equipe]);
$stats['victoires'] = $stmt->fetchColumn();

// Defeats by this team
$stmt = $pdo->prepare("SELECT COUNT(*) FROM infos_match WHERE id_equipe = :id AND match_gagnant = 'adverse'");
$stmt->execute(['id' => $id_equipe]);
$stats['defaites'] = $stmt->fetchColumn();

// Draws by this team
$stmt = $pdo->prepare("SELECT COUNT(*) FROM infos_match WHERE id_equipe = :id AND match_gagnant = 'match nul'");
$stmt->execute(['id' => $id_equipe]);
...
$stmt = $pdo->prepare("
    ...
    JOIN infos_match m ON a.id_match = m.id_match
    WHERE m.id_equipe = :id
...
<?php 
    ...
    <div class="header-ruban">
        <div class="ruban-section">
            ...
            ?>
                <a href="pg_stats.php?id_eq=<?= $equipe['id_equipe'] ?>"
...
<h2>⏱ Top 5 joueurs - Moyenne de minutes jouées</h2>
<table>
    <thead>
        <tr>
            <th>Nom</th>
